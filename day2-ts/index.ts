...
function reportIsDecreasing(report: Array<number>): boolean {
...
function reportIsIncreasing(report: Array<number>): boolean {
    for (let i = 0; i < report.length - 1; i++) {
        if (report[i] >= report[i + 1]) return false
    }
    return true
...
function reportLevelsClose(report: Array<number>): boolean {
    for (let i = 0; i < report.length - 1; i++) {
...
function checkSafeReport(report: Array<number>, withDampener: boolean): boolean {
    if (!withDampener) {
        const increasing = reportIsIncreasing(report)
        const decreasing = reportIsDecreasing(report)
        const closeLevels = reportLevelsClose(report)
        if ((increasing || decreasing) && closeLevels) return true
        return false
    }

    // Generate all permutations of reports to check
    for (let skip = 0; skip < report.length; skip++) {
        const reportWithSkipped = skip === 0 ? report.slice(1, report.length)
            : skip === report.length - 1 ? report.slice(0, report.length - 1)
            : [...report.slice(0, skip), ...report.slice(skip + 1, report.length)]
        const increasing = reportIsIncreasing(reportWithSkipped)
        const decreasing = reportIsDecreasing(reportWithSkipped)
        const closeLevels = reportLevelsClose(reportWithSkipped)
        if ((increasing || decreasing) && closeLevels) return true
    }
    return false
}

function numberOfSafeReports(reports: Array<Array<number>>, withDampener = false): number {
    return reports.filter(r => checkSafeReport(r, withDampener)).length
}

console.log(numberOfSafeReports(testReports))
console.log(numberOfSafeReports(testReports, true))
...
console.log(numberOfSafeReports(programInput))
console.log(numberOfSafeReports(programInput, true))
